...
var NUMERIC_INPUTS = [FIELDS.DOCUMENT_NUMBER, FIELDS.PHONE];
...
window.app = new Vue({
  ...
  computed: {
    ...
    subscriptionTotal: function () {
      return (
        this.methodSubtotal + this.productsSubtotal + this.deliverySubtotal
      );
    },
    onlyOneTime: function () {
      return (
        this.methodSubtotal + this.productsSubtotal == 0 &&
    ...
    firstDeliveryTotal: function () {
      ...
      if (
        ...
        discountValue =
          ...
          this.coupon.discount.percentage * ticketValueForDiscounts;
        discountValue = Math.min(
          discountValue,
          this.coupon.conditions.maximumDiscountValue || Infinity,
          ticketValueForDiscounts
        );
      }
    ...
    discountValue: function () {
      return (
        this.subscriptionTotal + this.oneTimeAmount - this.firstDeliveryTotal
      );
    },
    subDetails: function () {
      return {
  ...
  methods: {
    ...
    hasErrors: function (index) {
      return this.getErrors(index);
    }, // Se mantiene por motivos de compatibilidad
    ...
    hasOneTime: function () {
      const productsFilter = this.subscription.products.filter(
        (product) => product.is_once
      );
      const productsMap =
        productsFilter.length > 0
          ? productsFilter.map((item) => item.quantity)
          : [];
      return (
        (productsMap.length > 0
          ? productsMap.reduce((prev, curr) => prev + curr, 0)
          : 0) > 0
      );
    },
    updateCart: function (productData, event) {
      ...
      if (Number.isInteger(quantity)) {
        var cartItemIndex = this.subscription.products.findIndex(function (
          ...
          return product.slug == productData.slug;
        });
    ...
    showTotal: function (caseValue) {
      ...
      switch (caseValue) {
        ...
        case 3:
          visible =
            (this.methodSubtotal != 0 || this.productsSubtotal != 0) &&
            this.oneTimeAmount == 0
              ? true
              : false;
          break;
